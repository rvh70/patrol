...
#### iOS Develop Mode Auto-Restart

On iOS, the test runner process may occasionally timeout or lose connection,
especially after extended develop sessions. Patrol automatically restarts
the iOS test runner when this happens to keep your develop session alive.

You can configure the restart behavior using environment variables:

**PATROL_IOS_DEVELOP_MAX_RESTARTS** (default: `10`)

Maximum number of times to restart the iOS test runner before giving up.
Set to `0` to disable automatic restarts.

```
export PATROL_IOS_DEVELOP_MAX_RESTARTS=20
patrol develop --target patrol_test/example_test.dart
```

**PATROL_IOS_DEVELOP_INITIAL_BACKOFF_MS** (default: `2000`)

Initial delay in milliseconds before restarting the iOS test runner.
Subsequent restarts use exponential backoff (capped at 30 seconds).

```
export PATROL_IOS_DEVELOP_INITIAL_BACKOFF_MS=5000
patrol develop --target patrol_test/example_test.dart
```

These settings help handle transient issues like:
- "Test runner timed out while preparing to run tests"
- Connection failures between test runner and app
- Xcode build process issues

The automatic restart feature is designed to make `patrol develop` on iOS
more robust during long development sessions.
